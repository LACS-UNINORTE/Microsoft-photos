...
def add_face_data(fileID,vector,personID,x, y, w, h):
    connection = connect_db()
    with connection.cursor() as cur:
        cur.execute("INSERT INTO `facelacs`.`facedata` (`FileID`, `Vector`,`PersonID`,`x,y,w,h`) VALUES (%s,%s,%s,%s);",(fileID,vector,personID,f"{x},{y},{w},{h}"))
        connection.commit()
    cur.close() 
...
def get_photo_db():
    connection = connect_db()
    with connection.cursor() as cur:
        cur.execute("SELECT ID,Photo,PersonName FROM facelacs.`facereco` ;")
        admin = cur.fetchall()
    cur.close() 
    return admin

#filtrar fotos por caras

def get_photo_id_db(id):
    connection = connect_db()
    with connection.cursor() as cur:
        cur.execute("SELECT Photo,PersonName FROM facelacs.facereco WHERE ID = {};".format(id) )
        admin = cur.fetchall()
    cur.close() 
    return admin
...
def get_id_wname_db(name):
    connection = connect_db()
    with connection.cursor() as cur:
        cur.execute("SELECT ID FROM facelacs.`facereco` WHERE PersonName = '{}' ;".format(name))
        admin = cur.fetchall()
    cur.close() 
    return admin
...
#Darle nombre a la cara reconocida

def add_name_db(id,nombre):
    connection = connect_db()
    with connection.cursor() as cur:
        #cur.execute("INSERT INTO facelacs.facereco  WHERE (ID = {}) PersonName = {} ;".format(id,nombre))
        cur.execute("UPDATE `facelacs`.`facereco` SET `PersonName` = '{}' WHERE (`ID` = '{}');".format(nombre,id))
        connection.commit()
        cur.execute("UPDATE `facelacs`.`facedata` SET `PersonName` = '{}' WHERE (`PersonID` = '{}');".format(nombre,id))
        connection.commit()
        message = 'nombre agregado correctamente'
    cur.close() 
    return 

#filtrar fotos por nombre

def get_related_faces_db(FileID):
    connection = connect_db()
    with connection.cursor() as cur:
        cur.execute("SELECT PersonID,`x,y,w,h` FROM facelacs.facedata WHERE FileID = '{}' ;".format(FileID))
        admin = cur.fetchall()
    cur.close() 
    return admin

def get_photo_BID_db(ID):
    connection = connect_db()
    with connection.cursor() as cur:
        cur.execute("SELECT ID,Photo,PersonName FROM facelacs.`facereco` WHERE ID = {} ;".format(ID))
        admin = cur.fetchall()
    cur.close() 
    return admin
